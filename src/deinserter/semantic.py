...
def _write_po_from_mo(path: Path, destination: Path) -> None:
...
def _extract_glb_json(path: Path, destination: Path) -> None:
    data = path.read_bytes()
    ...
    while cursor + 8 <= total_length:
        ...
        if chunk_type == 0x4E4F534A:
            destination.write_bytes(data[cursor:end].rstrip(b" \0"))
            return
        cursor = (end + 3) & ~3
    raise ValueError("glb_json_chunk_missing")


def _write_generic_pseudocode(path: Path, destination: Path) -> None:
...
def _write_unreal_pseudocode(path: Path, destination: Path, parse_info: dict[str, Any]) -> None:
    payload = {
...
def build_semantic_conversion(
    ...
    try:
        if spec.strategy == "mo_to_po":
            ...
            _write_po_from_mo(file_path, destination)
            item.update({"status": "converted", "output_path": str(destination)})
        elif spec.strategy == "extract_glb_json_chunk":
            destination = semantic_output_base(root, file_path, output_dir, ".gltf.json", overwrite)
            _extract_glb_json(file_path, destination)
            item.update({"status": "converted", "output_path": str(destination)})
            ...
            _write_generic_pseudocode(file_path, destination)
            item.update({"status": "pseudocode", "output_path": str(destination)})
        elif spec.strategy == "unreal_header_pseudocode":
            destination = semantic_output_base(root, file_path, output_dir, ".semantic.json", overwrite)
            _write_unreal_pseudocode(file_path, destination, parse_info)
            item.update({"status": "pseudocode", "output_path": str(destination)})
